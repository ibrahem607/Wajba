...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Wajba.HttpApi.Host` and `Wajba.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 5bc2b726-f4bb-489b-b74c-788f5b4d4d8d
```

> `5bc2b726-f4bb-489b-b74c-788f5b4d4d8d` is the password of the certificate, you can change it to any password you want.
...
Run `Wajba.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Wajba.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Wajba.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
